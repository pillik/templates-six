...
{else}
    <script type="text/javascript">
        {$gateway->getHeadOutput()}
    </script>
    
    <form id="frmPayment" method="post" action="rccreditcard.php" class="form-horizontal" role="form">
            ...
            <div class="col-md-7">
                ...
                <div class="form-group">
                    <div class="col-sm-8 col-sm-offset-4">
                        <div class="radio">
                            <label>
                                <input type="radio" name="ccinfo" value="new" onclick="showNewCardInputFields()"{if $ccinfo eq "new" || !$cardOnFile} checked{/if} /> 
                ...
                <div class="form-group cc-billing-address{if !$userDetailsValidationError} hidden{/if}">
                    ...
                    <div class="col-sm-6">
                        <select class="form-control" id="inputCountry" name="country">
                            {foreach from=$countriesdropdown key=countrycode item=countryoption}
                                <option value="{$countrycode}" {if $country eq $countrycode}selected{/if}>{$countryoption}</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
